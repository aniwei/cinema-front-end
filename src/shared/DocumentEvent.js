...
const fire = (e, event) => {
  ...
  if (isArray(callbacks)) {
    ...
    !callbacks.length && document.removeEventListener(event, _eventHandlers[event], false);
  } else {
    callbacks.callback();

    if (callbacks.__ONCE__) {
      _eventCallbacks[event] = null;
      document.removeEventListener(event, _eventHandlers[event], false);
    }
  }
};
...
class WindowEvent {
  on(event, callback) {
    if (!_eventCallbacks[event]) {
      ...
      document.addEventListener(event, _eventHandlers[event], false);
    } else {
